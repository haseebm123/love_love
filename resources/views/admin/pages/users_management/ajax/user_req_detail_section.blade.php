            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-datas-center">
                    <div class="d-flex align-datas-center gap-10">
                        ...
                        @endphp
                        <div class="w-25">
                            <img src='{{ asset("documents/profile/$img") }}'
                                class="rounded-circle w-100" alt="Card image">
                        </div>

                        <div>
                            <h2>
                                {{ isset($data->first_name) ? $data->first_name.' ' :'' }} {{ isset($data->mid_name) ? $data->mid_name.' ' :'' }} {{ isset($data->last_name) ? $data->last_name.' ' :'' }}</h2>
                            <h6><img style="background-color: #F5D5E5;" src="{{ asset('icon/map-pin.svg') }}" >{{ isset($data->city) ? $data->city.' ,' :'' }}{{ isset($data->country) ? $data->country.' ' :'' }}</h6>
                        </div>
                    </div>
                    <div>
                        @if ($data->is_block == 0)
